...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 94608718CF
	for <lists+linux-can@lfdr.de>; Tue, 23 Jul 2019 15:00:18 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2390008AbfGWNAS (ORCPT <rfc822;lists+linux-can@lfdr.de>);
        Tue, 23 Jul 2019 09:00:18 -0400
Received: from mo4-p00-ob.smtp.rzone.de ([85.215.255.23]:19756 "EHLO
        mo4-p00-ob.smtp.rzone.de" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1731866AbfGWNAR (ORCPT
        <rfc822;linux-can@vger.kernel.org>); Tue, 23 Jul 2019 09:00:17 -0400
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; t=1563886812;
        s=strato-dkim-0002; d=hartkopp.net;
        h=Message-Id:Date:Subject:Cc:To:From:X-RZG-CLASS-ID:X-RZG-AUTH:From:
        Subject:Sender;
        bh=KufwNIpQpNvL6EPpA1/rVz5ME5CPjGztWW/gUXi1meo=;
        b=
X-RZG-AUTH: ":P2MHfkW8eP4Mre39l357AZT/I7AY/7nT2yrDxb8mjGrp7owjzFK3JbFk1mS0lO8DsfULo/u/TWqP+w=="
X-RZG-CLASS-ID: mo00
Received: from zbook.lan
        by smtp.strato.de (RZmta 44.24 DYNA|AUTH)
        with ESMTPSA id k05d3bv6ND0BeEh
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (curve secp521r1 with 521 ECDH bits, eq. 15360 bits RSA))
        (Client did not present a certificate);
        Tue, 23 Jul 2019 15:00:11 +0200 (CEST)
From:   Oliver Hartkopp <socketcan@hartkopp.net>
To:     linux-can@vger.kernel.org
Cc:     Oliver Hartkopp <socketcan@hartkopp.net>
Subject: [PATCH 1/2] can: gw: use struct canfd_frame as internal data structure
Date:   Tue, 23 Jul 2019 15:00:02 +0200
Message-Id: <20190723130003.17733-1-socketcan@hartkopp.net>
...
index d535fcc48055..aecccc15c972 100644
--- a/net/can/gw.c
+++ b/net/can/gw.c
...
@@ -177,12 +177,14 @@ static inline void canframecpy(struct can_frame *dst, struct can_frame *src)
 	 */
...
@@ -192,27 +194,27 @@ static int cgw_chk_csum_parms(s8 fr, s8 to, s8 re)
 	 * -8 => index = 0 (data[0])
 	 */
...
@@ -230,7 +232,7 @@ static void cgw_csum_xor_rel(struct can_frame *cf, struct cgw_csum_xor *xor)
 	cf->data[res] = val;
 }
...
+static void cgw_csum_xor_pos(struct canfd_frame *cf, struct cgw_csum_xor *xor)
 {
 	u8 val = xor->init_xor_val;
 	int i;
@@ -241,7 +243,7 @@ static void cgw_csum_xor_pos(struct can_frame *cf, struct cgw_csum_xor *xor)
...
+static void cgw_csum_xor_neg(struct canfd_frame *cf, struct cgw_csum_xor *xor)
 {
 	u8 val = xor->init_xor_val;
 	int i;
@@ -252,11 +254,11 @@ static void cgw_csum_xor_neg(struct can_frame *cf, struct cgw_csum_xor *xor)
 	cf->data[xor->result_idx] = val;
 }
...
@@ -291,7 +293,7 @@ static void cgw_csum_crc8_rel(struct can_frame *cf, struct cgw_csum_crc8 *crc8)
...
+static void cgw_csum_crc8_pos(struct canfd_frame *cf, struct cgw_csum_crc8 *crc8)
 {
 	u8 crc = crc8->init_crc_val;
 	int i;
@@ -318,7 +320,7 @@ static void cgw_csum_crc8_pos(struct can_frame *cf, struct cgw_csum_crc8 *crc8)
 	cf->data[crc8->result_idx] = crc^crc8->final_xor_val;
 }
...
+static void cgw_csum_crc8_neg(struct canfd_frame *cf, struct cgw_csum_crc8 *crc8)
 {
 	u8 crc = crc8->init_crc_val;
 	int i;
@@ -349,7 +351,7 @@ static void cgw_csum_crc8_neg(struct can_frame *cf, struct cgw_csum_crc8 *crc8)
...
@@ -411,7 +413,7 @@ static void can_can_gw_rcv(struct sk_buff *skb, void *data)
...
@@ -420,22 +422,22 @@ static void can_can_gw_rcv(struct sk_buff *skb, void *data)
...
@@ -506,7 +508,6 @@ static int cgw_notifier(struct notifier_block *nb,
...
@@ -543,32 +544,36 @@ static int cgw_put_job(struct sk_buff *skb, struct cgw_job *gwj, int type,
 			goto cancel;
 	}
...
+		}
 	...
 	if (gwj->mod.uid) {
@@ -656,7 +661,6 @@ static int cgw_parse_attr(struct nlmsghdr *nlh, struct cf_mod *mod,
...
@@ -676,69 +680,72 @@ static int cgw_parse_attr(struct nlmsghdr *nlh, struct cf_mod *mod,
 	}
