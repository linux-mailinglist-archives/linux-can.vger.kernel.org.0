...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 5D2FC704946
	for <lists+linux-can@lfdr.de>; Tue, 16 May 2023 11:30:38 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231567AbjEPJag (ORCPT <rfc822;lists+linux-can@lfdr.de>);
        Tue, 16 May 2023 05:30:36 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:42444 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S231558AbjEPJaf (ORCPT
        <rfc822;linux-can@vger.kernel.org>); Tue, 16 May 2023 05:30:35 -0400
Received: from mail-pf1-x429.google.com (mail-pf1-x429.google.com [IPv6:2607:f8b0:4864:20::429])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id DDF634227
        for <linux-can@vger.kernel.org>; Tue, 16 May 2023 02:30:30 -0700 (PDT)
Received: by mail-pf1-x429.google.com with SMTP id d2e1a72fcca58-64a9335a8e7so4171314b3a.0
        for <linux-can@vger.kernel.org>; Tue, 16 May 2023 02:30:30 -0700 (PDT)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=gmail.com; s=20221208; t=1684229430; x=1686821430;
        h=content-transfer-encoding:cc:to:subject:message-id:date:from
         :in-reply-to:references:mime-version:from:to:cc:subject:date
         :message-id:reply-to;
        bh=/yRMbmC/vXRKlKxXRdUnZ0bdODJzOjCzc5FULk7lonA=;
        b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=1e100.net; s=20221208; t=1684229430; x=1686821430;
        h=content-transfer-encoding:cc:to:subject:message-id:date:from
         :in-reply-to:references:mime-version:x-gm-message-state:from:to:cc
         :subject:date:message-id:reply-to;
        bh=/yRMbmC/vXRKlKxXRdUnZ0bdODJzOjCzc5FULk7lonA=;
        b=
X-Gm-Message-State: AC+VfDyiaGcksSuYGsk6LWeYVeWE+rRtFxLY0PStlHwS3olBY6jsB9vB
        mlMKF64qenzKwWj2MzSfkMCNLdOK0MJ4jJF/gfY1n/hlacM=
X-Google-Smtp-Source: ACHHUZ68j5tkV+ZD7S3jndADP8L7Y+d4MfYDqLx5HsBiToNXd+mOrFifx/trCCZrSEINhxqy9jhSEcsE7awX3rc1z6A=
X-Received: by 2002:a05:6a00:15c6:b0:635:d9e1:8e1e with SMTP id
 o6-20020a056a0015c600b00635d9e18e1emr52741424pfu.11.1684229429675; Tue, 16
 May 2023 02:30:29 -0700 (PDT)
MIME-Version: 1.0
References: <20230515212930.1019702-1-mkl@pengutronix.de>
In-Reply-To: <20230515212930.1019702-1-mkl@pengutronix.de>
From:   Vincent Mailhol <vincent.mailhol@gmail.com>
Date:   Tue, 16 May 2023 18:30:18 +0900
Message-ID: <CAMZ6RqKToYbfsOX4v_jK7X28hLyiYnH7j784eVeFuKrj7ujRSA@mail.gmail.com>
...
On Tue. 16 May 2023 =C3=A0 06:30, Marc Kleine-Budde <mkl@pengutronix.de> wr=
ote:
...
> diff --git a/drivers/net/can/usb/ixxat_usb/Makefile b/drivers/net/can/usb=
/ixxat_usb/Makefile
...
> diff --git a/drivers/net/can/usb/ixxat_usb/ixxat_usb_cl1.c b/drivers/net/=
can/usb/ixxat_usb/ixxat_usb_cl1.c
> new file mode 100644
...
> +       u8 opmode =3D IXXAT_USB_OPMODE_EXTENDED | IXXAT_USB_OPMODE_STANDA=
RD;
> +       u8 btr0 =3D ((bt->brp - 1) & 0x3f) | (((bt->sjw - 1) & 0x3) << 6)=
;
...
> +};
> diff --git a/drivers/net/can/usb/ixxat_usb/ixxat_usb_cl2.c b/drivers/net/=
can/usb/ixxat_usb/ixxat_usb_cl2.c
> new file mode 100644
...
> +       if (dev->can.ctrlmode & CAN_CTRLMODE_LISTENONLY)
> +               opmode |=3D IXXAT_USB_OPMODE_LISTONLY;
> +       if (dev->can.ctrlmode & (CAN_CTRLMODE_FD | CAN_CTRLMODE_FD_NON_IS=
O)) {
> +               exmode |=3D IXXAT_USB_EXMODE_EXTDATA | IXXAT_USB_EXMODE_F=
ASTDATA;
> +
...
> +               cmd->fdr.ts1 =3D cpu_to_le16(btd->prop_seg + btd->phase_s=
eg1);
> +               cmd->fdr.ts2 =3D cpu_to_le16(btd->phase_seg2);
> +               cmd->fdr.sjw =3D cpu_to_le16(btd->sjw);
> +               cmd->fdr.tdo =3D cpu_to_le16(btd->brp * (btd->phase_seg1 =
+ 1 +
> +                                                      btd->prop_seg));
> +       }
> +
...
> +       .init_ctrl =3D ixxat_usb_init_ctrl
> +};
> diff --git a/drivers/net/can/usb/ixxat_usb/ixxat_usb_core.c b/drivers/net=
/can/usb/ixxat_usb/ixxat_usb_core.c
> new file mode 100644
...
> +       { USB_DEVICE(IXXAT_USB_VENDOR_ID, USB2CAN_FD_PROFESSIONAL_PRODUCT=
_ID) },
> +       { USB_DEVICE(IXXAT_USB_VENDOR_ID, USB2CAN_FD_AUTOMOTIVE_PRODUCT_I=
D) },
...
For all your errors, use %pe to print the mnemonic instead of the error cod=
e.
...
> +               void *rb =3D res + pos;
> +
> +               ret =3D usb_control_msg(dev, rcp, rq, rti, port, 0, rb, r=
s, to);
> +               if (ret < 0) {
> +                       msleep(IXXAT_USB_MSG_CYCLE);
> +                       continue;
...
> +}
> +
> +static void ixxat_usb_update_ts_now(struct ixxat_usb_device *dev, u32 ts=
_now)
> +{
> +       u32 *ts_dev =3D &dev->time_ref.ts_dev_0;
> +       ktime_t *kt_host =3D &dev->time_ref.kt_host_0;
> +       u64 timebase =3D (u64)0x00000000FFFFFFFF - (u64)(*ts_dev) + (u64)=
ts_now;
...
> +}
> +
> +static void ixxat_usb_set_ts_now(struct ixxat_usb_device *dev, u32 ts_no=
w)
> +{
...
> +               dev_caps->bus_ctrl_types[i] =3D cmd->caps.bus_ctrl_types[=
i];
> +
> +fail:
> +       kfree(cmd);
...
> +               dev_info->device_fpga_version =3D cmd->info.device_fpga_v=
ersion;
...
> +}
> +
> +static int ixxat_usb_start_ctrl(struct ixxat_usb_device *dev, u32 *time_=
ref)
> +{
> +       const u16 port =3D dev->ctrl_index;
> +       int err;
...
> +               if (dev->tx_contexts[i].echo_index !=3D IXXAT_USB_MAX_TX_=
URBS) {
> +                       can_free_echo_skb(netdev, i, NULL);
> +                       dev->tx_contexts[i].echo_index =3D IXXAT_USB_MAX_=
TX_URBS;
> +               }
> +       }
> +}
...
> +}
> +
> +static int ixxat_usb_set_mode(struct net_device *netdev, enum can_mode m=
ode)
> +{
...
> +
> +       if (rx->base.size < (min_size - 1)) {
> +               netdev_err(netdev, "Error: Invalid can status message siz=
e\n");
> +               return -EBADMSG;
> +       }
> +
...
> +       else
> +               raw_status =3D le32_to_cpup((__le32 *)rx->cl2.data);
...
> +               dev->bec.rxerr =3D rx->cl1.data[IXXAT_USB_CAN_ERROR_COUNT=
ER_RX];
> +               dev->bec.txerr =3D rx->cl1.data[IXXAT_USB_CAN_ERROR_COUNT=
ER_TX];
> +       } else {
> +               raw_error =3D rx->cl2.data[IXXAT_USB_CAN_ERROR_CODE];
> +               dev->bec.rxerr =3D rx->cl2.data[IXXAT_USB_CAN_ERROR_COUNT=
ER_RX];
> +               dev->bec.txerr =3D rx->cl2.data[IXXAT_USB_CAN_ERROR_COUNT=
ER_TX];
> +       }
> +
...
> +               default:
> +                       netdev_err(netdev,
> +                                  "Unhandled rec type 0x%02x (%d): ignor=
ed\n",

Do not use parenthesis in log messages.

Ref: https://www.kernel.org/doc/html/latest/process/coding-style.html#print=
ing-kernel-messages
...
> +
> +               if (!(cf->can_id & CAN_RTR_FLAG) && (cf->flags & CANFD_BR=
S))
...
> +               netdev->stats.tx_bytes +=3D can_get_echo_skb(netdev, cont=
ext->echo_index, NULL);
> +       } else {
...
> +
> +       for (i =3D 0; i < IXXAT_USB_MAX_TX_URBS; i++) {
> +               if (dev->tx_contexts[i].echo_index =3D=3D IXXAT_USB_MAX_T=
X_URBS) {
...
> +               }
> +       } else {
> +               if (atomic_read(&dev->active_tx_urbs) >=3D IXXAT_USB_MAX_=
TX_URBS)
> +                       netif_stop_queue(netdev);
> +       }
> +
...
> +};
> +
> +static const struct ixxat_usb_adapter *ixxat_usb_get_adapter(const u16 i=
d)
> +{
> +       switch (id) {
...
can_idm structures into driver_info. Examples:
  ...
  https://elixir.bootlin.com/linux/v6.2/source/drivers/net/can/usb/peak_usb=
/pcan_usb_core.c#L30
...
Can you also populate ndev->ethtool_ops with the default timestamp info? c.=
f.
...
> +
> +       for (i =3D 0; i < IXXAT_USB_MAX_TX_URBS; i++)
> +               dev->tx_contexts[i].echo_index =3D IXXAT_USB_MAX_TX_URBS;
> +
...
> +       if (err) {
> +               dev_err(&intf->dev, "Error %d: Failed to register can dev=
ice\n",
> +                       err);
> +               goto free_candev;
> +       }
...
> +               const u8 epaddr =3D host_intf->endpoint[i].desc.bEndpoint=
Address;
> +               int match;
> +               u8 j;
> +
> +               /* Check if usb-endpoint address matches known usb-endpoi=
nts */
> +               for (j =3D 0; j < IXXAT_USB_MAX_CHANNEL; j++) {
> +                       u8 ep_msg_in =3D adapter->ep_msg_in[j];
> +                       u8 ep_msg_out =3D adapter->ep_msg_in[j];
> +
> +                       if (epaddr =3D=3D ep_msg_in || epaddr =3D=3D ep_m=
sg_out) {
> +                               match =3D 1;
> +                               break;
> +                       }
...
> +               u16 dev_bustype =3D le16_to_cpu(dev_caps.bus_ctrl_types[i=
]);
...
> diff --git a/drivers/net/can/usb/ixxat_usb/ixxat_usb_core.h b/drivers/net=
/can/usb/ixxat_usb/ixxat_usb_core.h
> new file mode 100644
...
> +
> +#define IXXAT_USB_DECODE_DLC(flags) (((flags) & IXXAT_USB_MSG_FLAGS_DLC)=
 >> 16)
> +#define IXXAT_USB_ENCODE_DLC(len) (((len) << 16) & IXXAT_USB_MSG_FLAGS_D=
LC)
...
> + * @flags: Message flags
> + *
> + * Contains the common fields of an IXXAT CAN message on both CL1 and CL=
2
> + * devices
> + */
...
It seems that your device supports TDC. What is the reason to not configure=
 it?

Please have a look at struct can_tdc:

  https://elixir.bootlin.com/linux/v6.2/source/include/linux/can/bittiming.=
h#L21
...
> + * @res: Response block
> + *
> + * This function sets the default values in the request and the response=
 block
> + * of a device command
> + */
> +void ixxat_usb_setup_cmd(struct ixxat_usb_dal_req *req,
