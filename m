Return-Path: <linux-can-owner@vger.kernel.org>
X-Original-To: lists+linux-can@lfdr.de
Delivered-To: lists+linux-can@lfdr.de
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 3937C32319A
	for <lists+linux-can@lfdr.de>; Tue, 23 Feb 2021 20:49:52 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S230330AbhBWTt0 (ORCPT <rfc822;lists+linux-can@lfdr.de>);
        Tue, 23 Feb 2021 14:49:26 -0500
Received: from mout02.posteo.de ([185.67.36.66]:43087 "EHLO mout02.posteo.de"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S230142AbhBWTtZ (ORCPT <rfc822;linux-can@vger.kernel.org>);
        Tue, 23 Feb 2021 14:49:25 -0500
Received: from submission (posteo.de [89.146.220.130]) 
        by mout02.posteo.de (Postfix) with ESMTPS id 460942400FD
        for <linux-can@vger.kernel.org>; Tue, 23 Feb 2021 20:48:27 +0100 (CET)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=posteo.de; s=2017;
        t=1614109707; bh=z//jpe7mKW4sT9/HZVp7VnhnL+4YbF1wOmB1D1Vf90o=;
        h=To:From:Autocrypt:Subject:Date:From;
        b=
Received: from customer (localhost [127.0.0.1])
        by submission (posteo.de) with ESMTPSA id 4DlV3G5wXpz6tmb
        for <linux-can@vger.kernel.org>; Tue, 23 Feb 2021 20:48:26 +0100 (CET)
To:     linux-can <linux-can@vger.kernel.org>
From:   Patrick Menschel <menschel.p@posteo.de>
Autocrypt: addr=menschel.p@posteo.de; prefer-encrypt=mutual; keydata=
 
Subject: Broadcast Manager tx behavior after bus-off
Message-ID: <bd5c2140-9639-75c2-ee00-16c528b2a486@posteo.de>
Date:   Tue, 23 Feb 2021 20:48:23 +0100
User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:68.0) Gecko/20100101
 Thunderbird/68.10.0
MIME-Version: 1.0
Content-Type: text/plain; charset=utf-8
Content-Language: en-US
Content-Transfer-Encoding: 8bit
Precedence: bulk
List-ID: <linux-can.vger.kernel.org>
X-Mailing-List: linux-can@vger.kernel.org

Hi,

I'd like to ask what the expected behavior of the Broadcast Manager's tx
path is after a bus-off occurs and the bus is restored.

My interface is configured with -restart-ms 100 .

From my observation, I believe the BCM does not restart tx by itself.

The background is, I'm running a custom pytest-suite with socketcan
tests against a device-under-test which is power-cycled during tests and
causes bus-off that way. I believe it's a miss-configuration though.

Thanks and Best Regards,
Patrick Menschel
